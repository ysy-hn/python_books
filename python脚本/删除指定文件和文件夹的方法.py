...
def get_file_path(root_path, file_list, dir_list):
    ...
    dir_or_files = os.listdir(root_path)  # 获取文件和文件夹名称并整理为列表
    for dir_file in dir_or_files:
        dir_file_path = os.path.join(root_path, dir_file)  # 获取文件夹或者文件的路径
...
def delete_file(file_list, del_file_lists, del_file=[]):
    """删除文件方法"""
    for file_name in file_list:
        if file_name in del_file_lists:
            del_file.append(file_name)
            os.remove(file_name)
    print("已删除的文件：{}".format(del_file))


def delete_dir(dir_list, del_dir_lists, del_dir=[]):
    """删除文件夹方法"""
    for dir_name in dir_list:
        if dir_name in del_dir_lists:
            del_dir.append(dir_name)
            shutil.rmtree(dir_name, True)
    print("已删除的文件夹：{}".format(del_dir))
...
if __name__ == "__main__":
    ...
    get_file_path(root_path, file_list, dir_list)  # 调用get_file_path获取文件和文件夹名方法
    delete_file(file_list, del_file_lists)  # 调用delete_file删除文件
    delete_dir(dir_list, del_dir_lists)  # 调用delete_dir删除文件夹方法
