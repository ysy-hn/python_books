...
class DeleteFileDir:
    ...
    def delete_file(self, del_file_lists):
        ...
        for file_name in self.file_list:
            if file_name.endswith('.pdb') or file_name.endswith('.obj') or file_name.endswith('.log') or file_name.endswith('.tlog'):
                os.remove(file_name)
...
# 指定要删除的文件名称
del_file_list = ['Config.xml', 'DALSA.SaperaLT.SapClassBasic.dll', 'Profile.dat', 'rdp_client.dll', 'pthreadVC2.dll',
                 'Recipes.dat', 'XQ.MicroFlash.exe.config']
# 指定要删除的文件夹名称
del_dir_list = ['config']
